...
class RecurrenceRule(models.Model):
    ...
    def _write_from_google(self, gevent, vals):
        ...
        for sleektiv_attendee_email in set(existing_attendees.mapped('email')):
            # Sometimes, several partners have the same email. Remove old attendees except organizer, otherwise the events will disappear.
            if email_normalize(sleektiv_attendee_email) not in emails:
                attendees = existing_attendees.exists().filtered(lambda att: att.email == email_normalize(sleektiv_attendee_email) and att.partner_id not in organizers_partner_ids)
