...
class Meeting(models.Model):
    ...
    def _sleektiv_attendee_commands(self, google_event):
        ...
        if google_event.exists(self.env):
            event = google_event.get_sleektiv_event(self.env)
        ...
        for sleektiv_attendee in attendees_by_emails.values():
            ...
            email = tools.email_normalize(sleektiv_attendee.email)
    ...
    def action_mass_archive(self, recurrence_update_setting):
        """ Delete recurrence in Sleektiv if in 'all_events' or in 'future_events' edge case, triggering one mail. """
        self.ensure_one()
    ...
    def _google_values(self):
        ...
        if self.user_id and self.user_id != self.env.user and not bool(self.user_id.sudo().google_calendar_token):
            # The organizer is an Sleektiv user that do not sync his calendar
            ...
            # the owner is not an Sleektiv user: We don't store the real owner identity (mail)
