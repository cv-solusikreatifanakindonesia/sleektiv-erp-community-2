...
from sleektiv.addons.google_account.models import google_service
...
class User(models.Model):
    ...
    def _check_pending_sleektiv_records(self):
    ...
    def _sync_google_calendar(self, calendar_service: GoogleCalendarService):
        ...
        if not results or (not results.get('events') and not self._check_pending_sleektiv_records()):
            return False
        ...
        # We apply Google updates only if their write date is later than the write date in Sleektiv.
        # It's possible that multiple updates affect the same record, maybe not directly.
        # To handle this, we preserve the write dates in Sleektiv before applying any updates,
        # and use these dates instead of the current live dates.
        sleektiv_events = self.env['calendar.event'].browse((events - recurrences).sleektiv_ids(self.env))
        sleektiv_recurrences = self.env['calendar.recurrence'].browse(recurrences.sleektiv_ids(self.env))
        recurrences_write_dates = {r.id: r.write_date for r in sleektiv_recurrences}
        events_write_dates = {e.id: e.write_date for e in sleektiv_events}
        synced_recurrences = self.env['calendar.recurrence']._sync_google2sleektiv(recurrences, recurrences_write_dates)
        synced_events = self.env['calendar.event']._sync_google2sleektiv(events - recurrences, events_write_dates, default_reminders=default_reminders)
    ...
    def _sync_single_event(self, calendar_service: GoogleCalendarService, sleektiv_event, event_id):
        self.ensure_one()
        ...
        synced_events = self.env['calendar.event']._sync_google2sleektiv(event, default_reminders=default_reminders)
        # Sleektiv -> Google
        sleektiv_event.with_context(send_updates=send_updates)._sync_sleektiv2google(calendar_service)
        return bool(sleektiv_event | synced_events)
