...
from sleektiv import api, Command, fields, SUPERUSER_ID
...
from sleektiv.tests import common, tagged, users
...
class TestMailMail(MailCommon):
    ...
    def test_mail_mail_send_exceptions_origin(self):
        ...
        for default_from in [False, '']:
            ...
            self._reset_data()
            with self.mock_mail_gateway(), mute_logger('sleektiv.addons.mail.models.mail_mail'):
...
class TestMailMailServer(MailCommon):
    ...
    def test_mail_mail_values_headers(self):
        """ Test headers content, notably X-Sleektiv-Message-Id added to keep context
        ...
        self.assertDictEqual(
            self._mails[0]['headers'],
            {
                ...
                'X-Sleektiv-Message-Id': message_id,
            }
        )
    ...
    @mute_logger('sleektiv.addons.mail.models.mail_mail')
    @patch('sleektiv.addons.base.models.ir_attachment.IrAttachment.file_size', new_callable=PropertyMock)
