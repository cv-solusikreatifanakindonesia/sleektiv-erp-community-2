...
class WebManifestRoutesTest(HttpCaseWithUserDemo):
    ...
    def test_webmanifest(self):
        ...
        data = response.json()
        self.assertEqual(data["name"], "Sleektiv")
        self.assertEqual(data["scope"], "/sleektiv")
        self.assertEqual(data["start_url"], "/sleektiv")
        self.assertEqual(data["display"], "standalone")
        self.assertEqual(data["background_color"], "#5f1c1c")
        self.assertEqual(data["theme_color"], "#5f1c1c")
        self.assertEqual(data["prefer_related_applications"], False)
        self.assertCountEqual(data["icons"], [
            {'src': '/web/static/img/sleektiv-icon-192x192.png', 'sizes': '192x192', 'type': 'image/png'},
            {'src': '/web/static/img/sleektiv-icon-512x512.png', 'sizes': '512x512', 'type': 'image/png'}
        ])
        ...
        for shortcut in data["shortcuts"]:
            ...
            self.assertTrue(shortcut["url"].startswith("/sleektiv?menu_id="))
    ...
    def test_webmanifest_scoped(self):
        ...
        self.assertEqual(data["prefer_related_applications"], False)
        self.assertCountEqual(data["icons"], [
            {'src': "/web/static/img/sleektiv-icon-192x192.png", 'sizes': 'any', 'type': 'image/png'}
        ])
        self.assertEqual(len(data["shortcuts"]), 0)
    ...
    def test_serviceworker(self):
        ...
        self.assertEqual(response.headers["Service-Worker-Allowed"], "/sleektiv")

    def test_offline_url(self):
        """
        This route returns the offline page
        """
        response = self.url_open("/sleektiv/offline")
        self.assertEqual(response.status_code, 200)
    ...
    def test_apple_touch_icon(self):
        ...
        response = self.url_open("/web/static/img/sleektiv-icon-ios.png")
        self.assertEqual(response.status_code, 200)

        document = self.url_open("/sleektiv")
        self.assertIn(
            '<link rel="apple-touch-icon" href="/web/static/img/sleektiv-icon-ios.png"/>', document.text,
