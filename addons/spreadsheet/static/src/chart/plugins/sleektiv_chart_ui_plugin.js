...
export class SleektivChartUIPlugin extends SleektivUIPlugin {
    ...
    constructor(config) {
        ...
        globalFiltersFieldMatchers["chart"] = {
            ...
            waitForReady: () => this._getSleektivChartsWaitForReady(),
    ...
    beforeHandle(cmd) {
        switch (cmd.type) {
            case "START":
                for (const chartId of this.getters.getSleektivChartIds()) {
        ...
        switch (cmd.type) {
            ...
            case "REDO": {
                ...
                for (const cmd of domainEditionCommands) {
                    if (!this.getters.getSleektivChartIds().includes(cmd.id)) {
                        continue;
                    }
                    const dataSource = this.getChartDataSource(cmd.id);
                    if (
    ...
    getChartDataSource(chartId) {
        const dataSourceId = this._getSleektivChartDataSourceId(chartId);
        return this.charts[dataSourceId];
    }

    /**
     * Get the callback used for onClick and onHover in an Sleektiv Chart
     */
     ...
     * @return {Promise[]}
     */
    _getSleektivChartsWaitForReady() {
        return this.getters
            .getSleektivChartIds()
            .map((chartId) => this.getChartDataSource(chartId).loadMetadata());
    }

    _getSleektivChartDataSourceId(chartId) {
     ...
     */
    _refreshSleektivCharts() {
        for (const chartId of this.getters.getSleektivChartIds()) {
            this._refreshSleektivChart(chartId);
        }
    }
}
