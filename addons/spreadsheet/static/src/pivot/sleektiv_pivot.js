...
import { NO_RECORD_AT_THIS_POSITION, SleektivPivotModel } from "./pivot_model";
import { EvaluationError, PivotRuntimeDefinition, registries, helpers } from "@sleektiv/o-spreadsheet";
import { LOADING_ERROR } from "@spreadsheet/data_sources/data_source";
import { omit } from "@web/core/utils/objects";
import { SleektivPivotLoader } from "./sleektiv_pivot_loader";
...
/**
 * @typedef {import("@sleektiv/o-spreadsheet").FunctionResultObject} FunctionResultObject
 * @typedef {import("@sleektiv/o-spreadsheet").PivotMeasure} PivotMeasure
 * @typedef {import("@sleektiv/o-spreadsheet").PivotDomain} PivotDomain
 * @typedef {import("@sleektiv/o-spreadsheet").PivotDimension} PivotDimension
 * @typedef {import("@sleektiv/o-spreadsheet").PivotCoreMeasure} PivotCoreMeasure
 * @typedef {import("@spreadsheet").WebPivotModelParams} WebPivotModelParams
 * @typedef {import("@spreadsheet").SleektivPivot<SleektivPivotRuntimeDefinition>} IPivot
 * @typedef {import("@spreadsheet").SleektivFields} SleektivFields
 * @typedef {import("@spreadsheet").SleektivPivotCoreDefinition} SleektivPivotCoreDefinition
 * @typedef {import("@spreadsheet").SortedColumn} SortedColumn
 * @typedef {import("@spreadsheet").SleektivGetters} SleektivGetters
 ...
 * @implements {IPivot}
 */
export class SleektivPivot {
    /**
     *
     * @override
     * @param {Object} services Services (see DataSource)
     * @param {Object} params
     * @param {SleektivPivotCoreDefinition} params.definition
     * @param {SleektivGetters} params.getters
     */
    constructor(services, { definition, getters }) {
        /** @type {"SLEEKTIV"} */
        this.type = "SLEEKTIV";

        /** @type {SleektivPivotCoreDefinition} @protected */
        this.coreDefinition = definition;

        this.needsReevaluation = false;

        /** @type {SleektivPivotRuntimeDefinition | undefined} @protected */
        this.runtimeDefinition = undefined;

        /** @type {SleektivPivotModel | undefined} @protected */
        this.model = undefined;

        /** @type {SleektivGetters} @protected */
        this.getters = getters;

        /** @protected */
        this.loader = new SleektivPivotLoader(services.sleektivDataProvider, this._load.bind(this));

        /** @type {SleektivFields | undefined} @protected */
        this._fields = undefined;

        /** @protected @type {SleektivDataProvider}*/
        this.sleektivDataProvider = services.sleektivDataProvider;
    ...
    /**
     * @param {SleektivPivotCoreDefinition} nextDefinition
     */
    onDefinitionChange(nextDefinition) {
        ...
        if (
            ...
            if (
                ...
                this.coreDefinition = nextDefinition;
                const runtimeDefinition = new SleektivPivotRuntimeDefinition(
                    this.coreDefinition,
                    this.getFields()
                );
    ...
    async createModelAndDefinition() {
        ...
        const definition = new SleektivPivotRuntimeDefinition(this.coreDefinition, this.getFields());
        const model = new SleektivPivotModel(
            { _t },
            {
            ...
            {
                orm: this.sleektivDataProvider.orm,
                serverData: this.sleektivDataProvider.serverData,
            }
        );
    ...
    /**
     * @param {import("@sleektiv/o-spreadsheet").Maybe<FunctionResultObject>[]} args
     *
     * @returns {PivotDomain}
     */
...
export class SleektivPivotRuntimeDefinition extends PivotRuntimeDefinition {
    /**
     * @param {SleektivPivotCoreDefinition} definition
     * @param {SleektivFields} fields
     */
...
supportedPivotPositionalFormulaRegistry.add("SLEEKTIV", true);
