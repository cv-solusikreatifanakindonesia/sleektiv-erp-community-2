...
import { SleektivBarChart } from "@spreadsheet/chart/sleektiv_chart/sleektiv_bar_chart";
import { SleektivChart } from "@spreadsheet/chart/sleektiv_chart/sleektiv_chart";
import { SleektivLineChart } from "@spreadsheet/chart/sleektiv_chart/sleektiv_line_chart";
...
test("Can add an Sleektiv Bar chart", async () => {
    ...
    expect(chart instanceof SleektivBarChart).toBe(true);
...
test("Can add an Sleektiv Line chart", async () => {
    const { model } = await createSpreadsheetWithChart({ type: "sleektiv_line" });
    const sheetId = model.getters.getActiveSheetId();
    expect(model.getters.getChartIds(sheetId).length).toBe(1);
    const chartId = model.getters.getChartIds(sheetId)[0];
    const chart = model.getters.getChart(chartId);
    expect(chart instanceof SleektivLineChart).toBe(true);
    expect(chart.getDefinitionForExcel()).toBe(undefined);
    expect(model.getters.getChartRuntime(chartId).chartJsConfig.type).toBe("line");
});

test("Can add an Sleektiv Pie chart", async () => {
    const { model } = await createSpreadsheetWithChart({ type: "sleektiv_pie" });
    const sheetId = model.getters.getActiveSheetId();
    expect(model.getters.getChartIds(sheetId).length).toBe(1);
    const chartId = model.getters.getChartIds(sheetId)[0];
    const chart = model.getters.getChart(chartId);
    expect(chart instanceof SleektivChart).toBe(true);
    expect(chart.getDefinitionForExcel()).toBe(undefined);
...
test("Sleektiv bar chart runtime loads the data", async () => {
    const { model } = await createSpreadsheetWithChart({
        type: "sleektiv_bar",
...
test("Sleektiv pie chart runtime loads the data", async () => {
    const { model } = await createSpreadsheetWithChart({
        type: "sleektiv_pie",
        mockRPC: async function (route, args) {
            if (args.method === "web_read_group") {
...
test("Sleektiv line chart runtime loads the data", async () => {
    const { model } = await createSpreadsheetWithChart({
        type: "sleektiv_line",
        mockRPC: async function (route, args) {
            if (args.method === "web_read_group") {
...
test("Can import/export an Sleektiv chart", async () => {
    const model = await createModelWithDataSource();
    insertChartInSpreadsheet(model, "sleektiv_line");
    const data = model.exportData();
    const figures = data.sheets[0].figures;
    expect(figures.length).toBe(1);
    const figure = figures[0];
    expect(figure.tag).toBe("chart");
    expect(figure.data.type).toBe("sleektiv_line");
...
test("Can undo/redo an Sleektiv chart creation", async () => {
    const model = await createModelWithDataSource();
    insertChartInSpreadsheet(model, "sleektiv_line");
    const sheetId = model.getters.getActiveSheetId();
    const chartId = model.getters.getChartIds(sheetId)[0];
...
test("charts with no legend", async () => {
    const { model } = await createSpreadsheetWithChart({ type: "sleektiv_pie" });
    insertChartInSpreadsheet(model, "sleektiv_bar");
    insertChartInSpreadsheet(model, "sleektiv_line");
    const sheetId = model.getters.getActiveSheetId();
...
test("Can copy/paste Sleektiv chart", async () => {
    const { model } = await createSpreadsheetWithChart({ type: "sleektiv_pie" });
    const sheetId = model.getters.getActiveSheetId();
    const chartId = model.getters.getChartIds(sheetId)[0];
    ...
    expect(model.getters.getChart(chartIds[1]) instanceof SleektivChart).toBe(true);
...
test("Can cut/paste Sleektiv chart", async () => {
    const { model } = await createSpreadsheetWithChart({ type: "sleektiv_pie" });
    const sheetId = model.getters.getActiveSheetId();
    const chartId = model.getters.getChartIds(sheetId)[0];
    ...
    expect(model.getters.getChart(chartIds[0]) instanceof SleektivChart).toBe(true);
    expect(JSON.stringify(model.getters.getChartRuntime(chartIds[0]))).toBe(
        JSON.stringify(chartRuntime)
    );
});

test("Duplicating a sheet correctly duplicates Sleektiv chart", async () => {
    const { model } = await createSpreadsheetWithChart({ type: "sleektiv_bar" });
    const sheetId = model.getters.getActiveSheetId();
...
test("Load sleektiv chart spreadsheet with models that cannot be accessed", async function () {
...
test("Can insert sleektiv chart from a different model", async () => {
    const model = await createModelWithDataSource();
...
test("Sleektiv chart legend color changes with background color update", async () => {
    const { model } = await createSpreadsheetWithChart({ type: "sleektiv_bar" });
    const sheetId = model.getters.getActiveSheetId();
    const chartId = model.getters.getChartIds(sheetId)[0];
    const definition = model.getters.getChartDefinition(chartId);
...
test("Remove sleektiv chart when sheet is deleted", async () => {
    const { model } = await createSpreadsheetWithChart({ type: "sleektiv_line" });
    const sheetId = model.getters.getActiveSheetId();
    ...
    expect(model.getters.getSleektivChartIds().length).toBe(1);
    model.dispatch("DELETE_SHEET", { sheetId });
    expect(model.getters.getSleektivChartIds().length).toBe(0);
});

test("Sleektiv chart datasource display name has a default when the chart title is empty", async () => {
    const { model } = await createSpreadsheetWithChart({ type: "sleektiv_line" });
    const sheetId = model.getters.getActiveSheetId();
    const chartId = model.getters.getChartIds(sheetId)[0];
    const definition = model.getters.getChartDefinition(chartId);
    expect(model.getters.getSleektivChartDisplayName(chartId)).toBe("(#1) Partners");
    model.dispatch("UPDATE_CHART", {
        definition: {
            ...definition,
            title: { text: "" },
    ...
    expect(model.getters.getSleektivChartDisplayName(chartId)).toBe("(#1) Sleektiv Line Chart");
});
...
test("Displays correct thousand separator for positive value in Sleektiv Bar chart Y-axis", async () => {
    const { model } = await createSpreadsheetWithChart({ type: "sleektiv_bar" });
    const sheetId = model.getters.getActiveSheetId();
    const chartId = model.getters.getChartIds(sheetId)[0];
    const runtime = model.getters.getChartRuntime(chartId);
...
test("Thousand separator in Sleektiv Bar chart Y-axis is locale-dependent", async () => {
    const { model } = await createSpreadsheetWithChart({ type: "sleektiv_bar" });
