...
import { advanceTime, animationFrame, tick, waitUntil } from "@sleektiv/hoot-dom";
...
class PeerTest {
    ...
    async focus() {
        return this.plugins["collaborationSleektiv"].joinPeerToPeer();
    }
    ...
    async writeToServer() {
        ...
        const lastId = this.plugins.collaborationSleektiv.getLastHistoryStepId(
        ...
        for (const peer of Object.values(this.peers)) {
            ...
            peer.onlineMutex.exec(async () => {
                return peer.plugins.collaborationSleektiv.onServerLastIdUpdate(String(lastId));
            });
        }
    }
    async setOnline() {
...
class Wysiwygs extends Component {
    ...
    onLoad(peerId, editor) {
        ...
        editor.startPlugins = () => {
            ...
            patch(plugins["collaborationSleektiv"], {
                getMetadata() {
...
describe("Stale detection & recovery", () => {
    describe("detect stale while unfocused", () => {
        test("should do nothing until focus", async () => {
            ...
            expect(peers.p1.plugins.collaborationSleektiv.isDocumentStale).toBe(false, {
            ...
            expect(peers.p2.plugins.collaborationSleektiv.isDocumentStale).toBe(false, {
            ...
            expect(peers.p3.plugins.collaborationSleektiv.isDocumentStale).toBe(true, {
            ...
            expect(peers.p3.plugins.collaborationSleektiv.isDocumentStale).toBe(false, {
    ...
    describe("detect stale while focused", () => {
        describe("recover from missing steps", () => {
            test("should recover from missing steps", async () => {
                ...
                const p3Spies = makeSpies(peers.p3.plugins.collaborationSleektiv, [
                ...
                expect(peers.p1.plugins.collaborationSleektiv.historyShareId).toBe(
                    peers.p2.plugins.collaborationSleektiv.historyShareId,
                    {
                        message: "p1 and p2 should have the same historyShareId",
                    }
                );
                expect(peers.p1.plugins.collaborationSleektiv.historyShareId).toBe(
                    peers.p3.plugins.collaborationSleektiv.historyShareId,
                    {
        ...
        describe("recover from snapshot", () => {
            test("should wait for all peer to recover from snapshot", async () => {
                ...
                const p2Spies = makeSpies(peers.p2.plugins.collaborationSleektiv, [
                    "recoverFromStaleDocument",
                    "resetFromServerAndResyncWithPeers",
                    "processMissingSteps",
                    "applySnapshot",
...
describe("Disconnect & reconnect", () => {
    test("should sync history when disconnecting and reconnecting to internet", async () => {
        ...
        const p1PromiseForMissingStep = new Promise((resolve) => {
            patch(peers.p2.plugins.collaborationSleektiv, {
        ...
        const p2PromiseForMissingStep = new Promise((resolve) => {
            patch(peers.p1.plugins.collaborationSleektiv, {
                async processMissingSteps() {
                    super.processMissingSteps(...arguments);
                    resolve();
                },
...
describe("History steps Ids", () => {
    ...
    test("should clear history step ids when resetting from server", async () => {
        ...
        expect(peers.p2.plugins.collaborationSleektiv.isDocumentStale).toBe(true, {
