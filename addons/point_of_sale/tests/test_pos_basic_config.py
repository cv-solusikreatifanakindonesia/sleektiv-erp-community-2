...
class TestPoSBasicConfig(TestPoSCommon):
    ...
    def test_cash_register_if_no_order(self):
        ...
        with (
            self.assertLogs('sleektiv.addons.point_of_sale.models.pos_order', level='DEBUG') as cm,
            unittest.mock.patch('sleektiv.addons.point_of_sale.models.pos_order.randrange', return_value=1996)
        ):
            ...
            sleektiv_order_id = res['pos.order'][0]['id']
            self.assertEqual(len(cm.output), 4)
            self.assertEqual(cm.output[0], f"INFO:sleektiv.addons.point_of_sale.models.pos_order:PoS synchronisation #1996 started for PoS orders references: [{order_log_str}]")
            self.assertTrue(cm.output[1].startswith(f'DEBUG:sleektiv.addons.point_of_sale.models.pos_order:PoS synchronisation #1996 processing order {order_log_str} order full data: '))
            self.assertEqual(cm.output[2], f'INFO:sleektiv.addons.point_of_sale.models.pos_order:PoS synchronisation #1996 order {order_log_str} created pos.order #{sleektiv_order_id}')
            self.assertEqual(cm.output[3], 'INFO:sleektiv.addons.point_of_sale.models.pos_order:PoS synchronisation #1996 finished')
