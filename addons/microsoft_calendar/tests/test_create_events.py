...
from sleektiv.addons.microsoft_calendar.tests.common import TestCommon, mock_get_token, _modified_date_in_the_future
...
class TestCreateEvents(TestCommon):
    ...
    def test_create_simple_event_without_sync(self, mock_insert):
        """
        A Sleektiv event is created when Outlook sync is not enabled.
        """
    ...
    @patch.object(MicrosoftCalendarService, 'get_events')
    def test_create_simple_event_from_outlook_attendee_calendar_and_organizer_does_not_exist_in_sleektiv(self, mock_get_events):
        """
        An event has been created in Outlook and synced in the Sleektiv attendee calendar.
        ...
        outlook_event = dict(self.simple_event_from_outlook_attendee, organizer={
            'emailAddress': {'address': "john.doe@sleektiv.com", 'name': "John Doe"},
        })
    ...
    def test_create_simple_event_from_outlook_attendee_calendar_where_email_addresses_are_capitalized(self, mock_get_events):
        """
        An event has been created in Outlook and synced in the Sleektiv attendee calendar.
        The email addresses of the attendee and the organizer are in different case than in Sleektiv.
        """
    ...
    def test_create_recurrent_event_without_sync(self, mock_insert):
        """
        A Sleektiv recurrent event is created when Outlook sync is not enabled.
        """
    ...
    def test_forbid_recurrences_creation_synced_outlook_calendar(self, mock_insert):
        """
        Forbids new recurrences creation in Sleektiv due to Outlook spam limitation of updating recurrent events.
        """
    ...
    def test_create_event_with_sync_config_paused(self, mock_insert):
        ...
        # Try to create a simple event in Sleektiv Calendar.
        record = self.env["calendar.event"].with_user(self.organizer_user).create(self.simple_event_values)
        self.call_post_commit_hooks()
        record.invalidate_recordset()
    ...
    def test_create_event_for_another_user(self, mock_insert, mock_get_events):
        """
        Allow the creation of event for another user only if the proposed user have its Sleektiv Calendar synced.
        ...
        # In Sleektiv, this event will be created (behind the screens) by a synced Sleektiv user as attendee (self.attendee_user).
        ...
        # Mock event from Microsoft and sync event with Sleektiv through self.attendee_user (synced user).
        ...
        # Ensure that event was successfully created in Sleektiv (no ACL error was triggered blocking creation).
    ...
    def test_create_simple_event_from_outlook_without_organizer(self, mock_get_events):
        """
        Allow creation of an event without organizer in Outlook and sync it in Sleektiv.
        """
    ...
    def test_create_event_with_default_and_undefined_sensitivity(self):
        """ Check if microsoft events are created in Sleektiv when 'None' sensitivity setting is defined and also when it is not. """
        # Sync events from Microsoft to Sleektiv after adding the sensitivity (privacy) property.
        ...
        self.env['calendar.event']._sync_microsoft2sleektiv(MicrosoftEvent([undefined_privacy_event, default_privacy_event]))

        # Ensure that synced events have the correct privacy field in Sleektiv.
        undefined_privacy_sleektiv_event = self.env['calendar.event'].search([('microsoft_id', '=', 100)])
        default_privacy_sleektiv_event = self.env['calendar.event'].search([('microsoft_id', '=', 200)])
        ...
        self.assertFalse(default_privacy_sleektiv_event.privacy, "Event with custom privacy must have False value in privacy field.")

    @patch.object(MicrosoftCalendarService, 'get_events')
    @patch.object(MicrosoftCalendarService, 'insert')
    def test_create_videocall_sync_microsoft_calendar(self, mock_insert, mock_get_events):
        """
        Test syncing an event from Sleektiv to Microsoft Calendar.
        ...
        # Verify the event's videocall_location is updated in Sleektiv
    ...
    def test_skip_sync_for_non_synchronized_users_new_events(self, mock_insert):
        ...
        Sleektiv, as well cause problems in the future the synchronization of that event for the original owner.
        """
        ...
        self.assertTrue(event, "The event for the not synchronized owner must be created in Sleektiv.")

        # Synchronize the calendar of user A, then make sure insert was not called.
        event.with_user(self.organizer_user).sudo()._sync_sleektiv2microsoft()
        mock_insert.assert_not_called()
...
class TestSyncSleektiv2MicrosoftMail(TestCommon, MailCommon):
    @classmethod
    def setUpClass(cls):
        super().setUpClass()
        cls.users = []
