...
:class:`~sleektiv.http.Controller`: class and its companion the
:func:`~sleektiv.http.route`: method decorator. Together they are used to
...
HTTP methods). See
https://www.sleektiv.com/documentation/master/developer/reference/addons/http.html#csrf
for more details.
...
def db_list(force=False, host=None):
    ...
    :param bool force: See :func:`~sleektiv.service.db.list_dbs`:
...
def dispatch_rpc(service_name, method, params):
    ...
    """
    rpc_dispatchers = {
        'common': sleektiv.service.common.dispatch,
        'db': sleektiv.service.db.dispatch,
        'object': sleektiv.service.model.dispatch,
    }
...
class Stream:
    ...
    def from_path(cls, path, filter_ext=('',), public=False):
        ...
        :param path: See :func:`~sleektiv.tools.file_path`
        :param filter_ext: See :func:`~sleektiv.tools.file_path`
    ...
    def get_response(
        ...
        **send_file_kwargs
        ...
        :param send_file_kwargs: Other keyword arguments to send to
            :func:`sleektiv.tools._vendor.send_file.send_file` instead of
...
class Controller:
    ...
    :class:`~sleektiv.http.Controller` can use the :func:`~sleektiv.http.route`:
    ...
    :func:`~sleektiv.http.route`:. Please note that the decorators of all
    ...
    .. code-block:
        ...
        class GreetingController(sleektiv.http.Controller):
    ...
    def __init_subclass__(cls):
        ...
        if Controller in cls.__bases__:
            ...
            module = path[2] if path[:2] == ['sleektiv', 'addons'] else ''
...
def route(route=None, **routing):
    ...
    .. warning::
        ...
        :class:`~sleektiv.http.Controller` for more details.
...
def _generate_routing_rules(modules, nodb_only, converters=None):
    ...
    def is_valid(cls):
        ...
        path = cls.__module__.split('.')
        return path[:2] == ['sleektiv', 'addons'] and path[2] in modules
    ...
    def build_controllers():
        ...
        """
        # Controllers defined outside of sleektiv addons are outside of the
        # controller inheritance/extension mechanism.
        yield from (ctrl() for ctrl in Controller.children_classes.get('', []))

        # Controllers defined inside of sleektiv addons can be extended in
...
class Session(collections.abc.MutableMapping):
    ...
    def authenticate(self, dbname, credential):
        ...
        with registry.cursor() as cr:
            env = sleektiv.api.Environment(cr, pre_uid, {})
        ...
        if request and request.session is self and request.db == dbname:
            request.env = sleektiv.api.Environment(request.env.cr, self.uid, self.context)
...
class GeoIP(collections.abc.Mapping):
    ...
    .. note:
        ...
        :attr:`~sleektiv.http.request.geoip`.

    .. code-block:

        >>> GeoIP('127.0.0.1').country.iso_code
        >>> sleektiv_ip = socket.gethostbyname('sleektiv.com')
        >>> GeoIP(sleektiv_ip).country.iso_code
        'FR'
    """
...
class Response(werkzeug.wrappers.Response):
    ...
    def load(cls, result, fname="<function>"):
        ...
        :returns: The created :class:`~sleektiv.http.Response`.
        :rtype: Response
...
class Request:
    ...
    def update_env(self, user=None, context=None, su=None):
        ...
        :type user: int or :class:`res.users record<~sleektiv.addons.base.models.res_users.Users>`
    ...
    def make_response(self, data, headers=None, cookies=None, status=200):
        ...
        :rtype: :class:`~sleektiv.http.Response`
        """
    ...
    def _serve_db(self):
        ...
        try:
            ...
            self.env = sleektiv.api.Environment(cr_readwrite, self.session.uid, self.session.context)
            try:
            ...
            with contextlib.closing(self.registry.cursor(readonly=readonly_cr)) as cr:
                ...
                try:
                    ...
                    if isinstance(exc, HTTPException) and exc.code is None:
                        raise  # bubble up to sleektiv.http.Application.__call__
...
class HttpDispatcher(Dispatcher):
    ...
    def dispatch(self, endpoint, args):
        ...
        See :meth:`~sleektiv.http.Response.load` method for the compatible
...
class Application:
    ...
    def nodb_routing_map(self):
        ...
        for url, endpoint in _generate_routing_rules([''] + sleektiv.conf.server_wide_modules, nodb_only=True):
    ...
    def __call__(self, environ, start_response):
        ...
        if sleektiv.tools.config['proxy_mode'] and environ.get("HTTP_X_FORWARDED_HOST"):
            ...
            try:
                ...
                if self.get_static_file(httprequest.path):
                    ...
                    try:
                        ...
                        if (httprequest.path.startswith('/sleektiv/')
                            or httprequest.path in (
                                '/sleektiv', '/web', '/web/login', '/test_http/ensure_db',
                            )):
