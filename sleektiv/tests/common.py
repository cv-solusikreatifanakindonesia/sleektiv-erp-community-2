...
from sleektiv.sql_db import BaseCursor, Cursor
from sleektiv.tools import config, float_compare, mute_logger, profiler, SQL, DotDict
from sleektiv.tools.mail import single_email_re
from sleektiv.tools.misc import find_in_path, lower_logging
from sleektiv.tools.xml_utils import _validate_xml
...
def __getattr__(name):
    ...
    warnings.warn(
        "Since 18.0: sleektiv.tests.common.Form is deprecated, use sleektiv.tests.Form",
...
class BaseCase(case.TestCase, metaclass=MetaCase):
    ...
    longMessage = True      # more verbose error message by default: https://www.sleektiv.com/r/Vmh
    ...
    def debug_mode(self):
        ...
        request = Mock(
            ...
            env=self.env,
            session=DotDict(sleektiv.http.get_default_session(), debug='1'),
        )
        try:
            self.env.flush_all()
            ...
            popped_request = sleektiv.http._request_stack.pop()
    ...
    def _patchExecute(self, actual_queries, flush=True):
        ...
        with (
            patch('sleektiv.sql_db.Cursor.execute', execute),
    ...
    def assertRecordValues(
            self,
            records: sleektiv.models.BaseModel,
...
class Approx:  # noqa: PLW1641
    ...
    """
    def __init__(self, value: float, rounding: int | float | sleektiv.addons.base.models.res_currency.Currency, /, decorate: bool) -> None:  # noqa: PYI041
        self.value = value
...
class TransactionCase(BaseCase):
    ...
    cr: Cursor = None
    muted_registry_logger = mute_logger(sleektiv.modules.registry._logger.name)
    freeze_time = None
    ...
    def _gc_filestore(cls):
        ...
        with Registry(get_db_name()).cursor() as cr:
            gc_env = api.Environment(cr, sleektiv.SUPERUSER_ID, {})
    ...
    def setUpClass(cls):
        ...
        cls._crypt_context_patcher = patch('sleektiv.addons.base.models.res_users.Users._crypt_context', _crypt_context)
...
def save_test_file(test_name, content, prefix, extension='png', logger=_logger, document_type='Screenshot', date_format="%Y%m%d_%H%M%S_%f"):
    ...
    screenshots_dir = pathlib.Path(sleektiv.tools.config['screenshots']) / get_db_name() / 'screenshots'
...
class HttpCase(TransactionCase):
    ...
    @classmethod
    def http_port(cls):
        if sleektiv.service.server.server is None:
            return None
        return sleektiv.service.server.server.httpd.server_port
    ...
    def authenticate(self, user, password, browser: ChromeBrowser = None):
        ...
        session.update(sleektiv.http.get_default_session(), db=get_db_name())
        session.context['lang'] = sleektiv.http.DEFAULT_LANG
        ...
        if user: # if authenticated
            ...
            with patch('sleektiv.addons.base.models.res_users.Users._check_credentials', new=patched_check_credentials):
    ...
    def start_tour(self, url_path, tour_name, step_delay=None, **kwargs):
        ...
        code = kwargs.pop('code', f"sleektiv.startTour({tour_name!r}, {json.dumps(options)})")
        ready = kwargs.pop('ready', f"sleektiv.isTourReady({tour_name!r})")
    ...
    def profile(self, **kwargs):
        ...
        return profiler.Nested(_profiler, patch('sleektiv.http.Request._get_profiler_context_manager', route_profiler))
