...
from sleektiv.tools.cache import log_ormcache_stats
from sleektiv.tools.misc import stripped_sys_argv, dumpstacks
...
def set_limit_memory_hard():
    ...
    if sleektiv.evented and config['limit_memory_hard_gevent']:
...
class ThreadedServer(CommonServer):
    ...
    def cron_thread(self, number):
        ...
        while True:
            conn = sleektiv.sql_db.db_connect('postgres')
    ...
    def run(self, preload=None, stop=False):
        ...
        if stop:
            if config['test_enable']:
                from sleektiv.tests.result import _logger as logger  # noqa: PLC0415
...
class WorkerCron(Worker):
    ...
    def start(self):
        ...
        dbconn = sleektiv.sql_db.db_connect('postgres')
...
def load_test_file_py(registry, test_file):
    ...
    from sleektiv.tests import loader  # noqa: PLC0415
    from sleektiv.tests.suite import OdooSuite  # noqa: PLC0415
...
def start(preload=None, stop=False):
    ...
    if sleektiv.evented:
        server = GeventServer(sleektiv.http.root)
    elif config['workers']:
        if config['test_enable'] or config['test_file']:
            _logger.warning("Unit testing in workers mode could fail; use --workers 0.")

        server = PreforkServer(sleektiv.http.root)
    else:
        ...
        server = ThreadedServer(sleektiv.http.root)
