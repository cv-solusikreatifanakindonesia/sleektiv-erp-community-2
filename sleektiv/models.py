...
if typing.TYPE_CHECKING:
    ...
    from sleektiv.api import Self, ValuesType, IdType
    ...
    def __new__(meta, name, bases, attrs):
        ...
        if attrs.get('_register', True):
            ...
            if '_module' not in attrs:
                ...
                assert module.startswith('sleektiv.addons.'), \
                    f"Invalid import of {module}.{name}, it should start with 'sleektiv.addons'."
...
class BaseModel(metaclass=MetaModel):
    ...
    *   :class:`AbstractModel` for abstract super classes meant to be shared by
    ...
    def load(self, fields, data):
        ...
        for field_path in fields:
            ...
            for field_name in field_path:
                ...
                if isinstance(model_fields.get(field_name), sleektiv.fields.One2many):
    ...
    def _compute_display_name(self):
        ...
        it can be made field-dependent using :attr:`~sleektiv.api.depends` and
        context-dependent using :attr:`~sleektiv.api.depends_context`.
        """
        if self._rec_name:
        ...
        if field.type in ('datetime', 'date') or (field.type == 'properties' and granularity):
            ...
            if granularity == 'week':
                ...
                if granularity == 'day_of_week':
                    """
                    formula: ((7 - first_day_of_week_in_sleektiv) + result_from_SQL) %  --> 0 based first day of week
                                    week start on
                                monday   sunday    sat
                                  1     |  7    |  6   <-- first day of week in sleektiv
                           SQL | -----------------------
                    Monday  1  |  0     |  1    |  2
                    tuesday 2  |  1     |  2    |  3
                    wed     3  |  2     |  3    |  4
    ...
    def _read_group_fill_temporal(self, data, groupby, annoted_aggregates,
        ...
        if fill_from:
            fill_from = sleektiv.fields.Datetime.to_datetime(fill_from) if isinstance(fill_from, datetime.datetime) else sleektiv.fields.Date.to_date(fill_from)
        ...
        if fill_to:
            fill_to = sleektiv.fields.Datetime.to_datetime(fill_to) if isinstance(fill_to, datetime.datetime) else sleektiv.fields.Date.to_date(fill_to)
    ...
    def _field_to_sql(self, alias: str, fname: str, query: (Query | None) = None, flush: bool = True) -> SQL:
        ...
        result to make method :meth:`~sleektiv.api.Environment.execute_query` flush
    ...
    def write(self, vals: ValuesType) -> typing.Literal[True]:
        ...
        * The expected value of a :class:`~sleektiv.fields.One2many` or
          :class:`~sleektiv.fields.Many2many` relational field is a list of
          :class:`~sleektiv.fields.Command` that manipulate the relation the
          implement. There are a total of 7 commands:
          :meth:`~sleektiv.fields.Command.create`,
          :meth:`~sleektiv.fields.Command.update`,
          :meth:`~sleektiv.fields.Command.delete`,
          :meth:`~sleektiv.fields.Command.unlink`,
          :meth:`~sleektiv.fields.Command.link`,
          :meth:`~sleektiv.fields.Command.clear`, and
          :meth:`~sleektiv.fields.Command.set`.
        * For :class:`~sleektiv.fields.Date` and `~sleektiv.fields.Datetime`,
