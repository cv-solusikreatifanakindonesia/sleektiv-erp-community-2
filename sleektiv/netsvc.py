...
def log(logger, level, prefix, msg, depth=None):
    warnings.warn(
        "sleektiv.netsvc.log is deprecated starting Odoo 18, use normal logging APIs",
...
class PostgreSQLHandler(logging.Handler):
    ...
    def emit(self, record):
        ...
        if not dbname:
            return
        with contextlib.suppress(Exception), tools.mute_logger('sleektiv.sql_db'), sql_db.db_connect(dbname, allow_uri=True).cursor() as cr:
...
def init_logger():
    ...
    warnings.filterwarnings('ignore', category=BytesWarning, module='sleektiv.tools.image')
