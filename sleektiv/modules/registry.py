...
import sleektiv
from sleektiv.modules.db import FunctionStatus
...
from sleektiv.tools.func import locked
from sleektiv.tools.lru import LRU
from sleektiv.tools.misc import Collector, format_frame
...
class Registry(Mapping):
    ...
    def new(cls, db_name, force_demo=False, status=None, update_module=False):
        ...
        try:
            ...
            try:
                sleektiv.modules.load_modules(registry, force_demo, status, update_module)
            except Exception:
    ...
    def init(self, db_name):
        ...
        if config['test_enable'] or config['test_file']:
            from sleektiv.tests.result import OdooTestResult  # noqa: PLC0415
        ...
        self._db = sleektiv.sql_db.db_connect(db_name, readonly=False)
        ...
        if config['db_replica_host'] is not False or config['test_enable']:  # by default, only use readonly pool if we have a db_replica_host defined. Allows to have an empty replica host for testing
            self._db_readonly = sleektiv.sql_db.db_connect(db_name, readonly=True)
        ...
        with closing(self.cursor()) as cr:
            ...
            self.has_trigram = sleektiv.modules.db.has_trigram(cr)
