...
PYTHON_TRANSLATION_COMMENT = 'sleektiv-python'

# translation used for javascript code in web client
JAVASCRIPT_TRANSLATION_COMMENT = 'sleektiv-javascript'
...
def get_translated_module(arg: str | int | typing.Any) -> str:  # frame not represented as hint
    ...
    if isinstance(arg, str):
        if arg.startswith('sleektiv.addons.'):
            # get the name of the module
            return arg.split('.')[2]
        if '.' in arg or not arg:
            # module name is not in sleektiv.addons.
            return 'base'
        else:
            return arg
    else:
        ...
        if not frame:
            return 'base'
        if (module_name := frame.f_globals.get("__name__")) and module_name.startswith('sleektiv.addons.'):
            # just a quick lookup because `get_resource_from_path is slow compared to this`
            return module_name.split('.')[2]
        path = inspect.getfile(frame)
        path_info = sleektiv.modules.get_resource_from_path(path)
...
def _get_cr(frame):
    ...
    if (local_self := frame.f_locals.get('self')) is not None:
        ...
        if (cr := getattr(local_self, 'cr', None)) is not None:
            return cr
    try:
        from sleektiv.http import request  # noqa: PLC0415
            ...
            for root, dummy, files in os.walk(path, followlinks=True):
                ...
                if fnmatch.fnmatch(root, '*/data/*'):
                    for fname in filter(spreadsheet_files_regex.match, files):
                        self._babel_extract_terms(fname, path, root, 'sleektiv.tools.translate:extract_spreadsheet_terms',
                                                  extra_comments=[JAVASCRIPT_TRANSLATION_COMMENT])
                if not recursive:
...
def get_po_paths(module_name: str, lang: str, env: sleektiv.api.Environment | None = None):
...
def _get_translation_upgrade_queries(cr, field):
    ...
    """
    from sleektiv.modules.registry import Registry  # noqa: PLC0415
