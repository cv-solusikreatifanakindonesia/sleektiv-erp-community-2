"""
This code is what let us use ES6-style modules in sleektiv.
Classic Odoo modules are composed of a top-level :samp:`sleektiv.define({name},{dependencies},{body_function})` call.
This processor will take files starting with an `@sleektiv-module` annotation (in a comment) and convert them to classic modules.
If any file has the ``/** sleektiv-module */`` on top of it, it will get processed by this class.
It performs several operations to get from ES6 syntax to the usual sleektiv one with minimal changes.
...
def transpile_javascript(url, content):
    """
    Transpile the code from native JS modules to custom sleektiv modules.
    ...
    legacy_sleektiv_define = get_aliased_sleektiv_define_content(module_path, content)
    ...
    steps = [
        ...
        partial(wrap_with_sleektiv_define, module_path, dependencies),
    ]
    for s in steps:
        content = s(content)
    if legacy_sleektiv_define:
        content += legacy_sleektiv_define
    return content
...
def url_to_module_path(url):
    """
    Odoo modules each have a name. (sleektiv.define("<the name>", [<dependencies>], function (require) {...});
...
def wrap_with_sleektiv_define(module_path, dependencies, content):
    """
    Wraps the current content (source code) with the sleektiv.define call.
    ...
    """
    return f"""sleektiv.define({module_path!r}, {list(dependencies)}, function (require) {{
'use strict';
let __exports = {{}};
{content}
return __exports;
...
ODOO_MODULE_RE = re.compile(r"""
    ...
    @sleektiv-module                    # '@sleektiv-module' statement
...
def is_sleektiv_module(url, content):
    """
    Detect if the file is a native sleektiv module.
    We look for a comment containing @sleektiv-module.

    :param url:
    :param content: source code
    :return: is this a sleektiv module that need transpilation ?
    """
...
def get_aliased_sleektiv_define_content(module_path, content):
    """
    ...
    Example:
    If we have a require call somewhere in the sleektiv source base being:
    ...
    > sleektiv.define("web/chrome/abstract_action", ['web.AbstractAction'], function (require) {
    ...
    .. code-block:: javascript
        ...
        /** @sleektiv-module */
        // after
        /** @sleektiv-module alias=web.AbstractAction */
    ...
    .. code-block:: javascript
        ...
        /** @sleektiv-module */
        // after
        /** @sleektiv-module alias=web.AbstractAction default=false */
    ...
    if matchobj:
        ...
        if alias:
            if matchobj['default']:
                return """\nsleektiv.define(`%s`, ['%s'], function (require) {
                        return require('%s');
