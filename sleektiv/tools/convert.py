...
def _get_idref(self, env, model_str, idref):
    idref2 = dict(idref,
                  Command=sleektiv.fields.Command,
                  time=time,
                  DateTime=datetime,
                  datetime=datetime,
                  timedelta=timedelta,
    ...
    def _tag_menuitem(self, rec, parent=None):
        ...
        for group in rec.get('groups', '').split(','):
            if group.startswith('-'):
                ...
                groups.append(sleektiv.Command.unlink(group_id))
            elif group:
                group_id = self.id_get(group)
                groups.append(sleektiv.Command.link(group_id))
        if groups:
    ...
    def _tag_record(self, rec, extra_vals=None):
        ...
        for field in rec.iterchildren('field'):
            ...
            if f_search := field.get("search"):
                ...
                if (f_name in _fields) and _fields[f_name].type == 'many2many':
                    f_val = [sleektiv.Command.set([x[f_use] for x in s])]
                elif len(s):
    ...
    def _tag_root(self, el):
        for rec in el:
            ...
            try:
                ...
                raise ParseError(msg) from None  # Restart with "--log-handler sleektiv.tools.convert:DEBUG" for complete traceback
