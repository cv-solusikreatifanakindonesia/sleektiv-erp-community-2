...
from sleektiv.tools.arabic_reshaper import reshape
...
def add_banner(pdf_stream, text=None, logo=False, thickness=2 * cm):
    ...
    sleektiv_logo = Image.open(file_open('base/static/img/main_partner-image.png', mode='rb'))
    sleektiv_color = colors.Color(113 / 255, 75 / 255, 103 / 255, 0.8)
    ...
    for p in range(old_pdf.getNumPages()):
        ...
        can.setFillColor(sleektiv_color)
        ...
        logo and can.drawImage(
            ImageReader(sleektiv_logo), 0.25 * thickness, -2.05 * thickness, 40, 40, mask='auto', preserveAspectRatio=True)
