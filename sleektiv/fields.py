...
import typing
from sleektiv.api import ContextType, DomainType, IdType, NewId, M, T
...
class Field(MetaField('DummyField', (object,), {}), typing.Generic[T]):
    ...
    :param str aggregator: aggregate function used by :meth:`~sleektiv.models.Model.read_group`
...
class Many2one(_Relational[M]):
    ...
    :param bool check_company: Mark the field to be verified in
        :meth:`~sleektiv.models.Model._check_company`. Has a different behaviour
...
class Properties(Field):
    ...
    current record. The container must have a :class:`~sleektiv.fields.PropertiesDefinition`
...
class PropertiesDefinition(Field):
    """ Field used to define the properties definition (see :class:`~sleektiv.fields.Properties`
...
class Command(enum.IntEnum):
    """
    :class:`~sleektiv.fields.One2many` and :class:`~sleektiv.fields.Many2many` fields
    ...
    def create(cls, values: dict):
        ...
        In case of a :class:`~sleektiv.fields.Many2many` relation, one unique
        new record is created in the comodel such that all records in `self`
        are linked to the new record.

        In case of a :class:`~sleektiv.fields.One2many` relation, one new record
    ...
    def delete(cls, id: int):
        ...
        In case of a :class:`~sleektiv.fields.Many2many` relation, removing the
    ...
    def unlink(cls, id: int):
        ...
        In case of a :class:`~sleektiv.fields.One2many` relation, the given record
