...
from sleektiv.tools.cache import get_cache_key_counter
...
class TestOrmCache(TransactionCase):
    ...
    def test_signaling_01_single(self):
        ...
        with self.assertLogs('sleektiv.modules.registry') as logs:
        ...
        self.assertEqual(
            logs.output,
            ["INFO:sleektiv.modules.registry:Caches invalidated, signaling through the database: ['assets']"],
        )
        ...
        self.assertEqual(
            logs.output,
            ["INFO:sleektiv.modules.registry:Invalidating caches after database signaling: ['assets', 'templates.cached_values']"],
        )
    ...
    def test_signaling_01_multiple(self):
        ...
        self.assertEqual(
            logs.output,
            [
                "INFO:sleektiv.modules.registry:Caches invalidated, signaling through the database: ['assets', 'default']",
            ],
        )
        ...
        self.assertEqual(
            logs.output,
            ["INFO:sleektiv.modules.registry:Invalidating caches after database signaling: ['assets', 'default', 'templates.cached_values']"],
        )
