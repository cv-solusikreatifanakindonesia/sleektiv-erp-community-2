...
from sleektiv.modules.module import load_manifest
...
class TestModuleManifest(BaseCase):
    @classmethod
    def setUpClass(cls):
        cls._tmp_dir = tempfile.TemporaryDirectory(prefix='sleektiv-test-addons-')
        cls.addClassCleanup(cls._tmp_dir.cleanup)
        cls.addons_path = cls._tmp_dir.name

        patcher = patch.object(sleektiv.addons, '__path__', [cls.addons_path])
        cls.startClassPatcher(patcher)

    def setUp(self):
        self.module_root = tempfile.mkdtemp(prefix='sleektiv-test-module-', dir=self.addons_path)
    ...
    def test_default_manifest(self):
        ...
        with self.assertNoLogs('sleektiv.modules.module', 'WARNING'):
    ...
    def test_missing_manifest(self):
        with self.assertLogs('sleektiv.modules.module', 'DEBUG') as capture:
            manifest = load_manifest(self.module_name)
    ...
    def test_missing_license(self):
        ...
        with self.assertLogs('sleektiv.modules.module', 'WARNING') as capture:
            manifest = load_manifest(self.module_name)
