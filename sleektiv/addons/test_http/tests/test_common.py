...
import sleektiv
from sleektiv.http import Session
from sleektiv.addons.base.tests.common import HttpCaseWithUserDemo
from sleektiv.tools.func import lazy_property
from sleektiv.addons.test_http.utils import MemoryGeoipResolver, MemorySessionStore
...
class TestHttpBase(HttpCaseWithUserDemo):
    ...
    def setUpClass(cls):
        ...
        cls.classPatch(sleektiv.http.Application, 'session_store', session_store)
        cls.classPatch(sleektiv.http.Application, 'geoip_city_db', geoip_resolver)
        cls.classPatch(sleektiv.http.Application, 'geoip_country_db', geoip_resolver)

    def setUp(self):
        super().setUp()
        sleektiv.http.root.session_store.store.clear()
    ...
    def nodb_url_open(self, url, *args, allow_redirects=False, **kwargs):
        with patch('sleektiv.http.db_list') as db_list, \
             patch('sleektiv.http.db_filter') as db_filter:
    ...
    def multidb_url_open(self, url, *args, allow_redirects=False, dblist=(), **kwargs):
        ...
        with patch('sleektiv.http.db_list') as db_list, \
             patch('sleektiv.http.db_filter') as db_filter, \
             patch('sleektiv.http.Registry') as Registry:
