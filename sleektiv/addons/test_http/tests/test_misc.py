...
import sleektiv
from sleektiv.http import root, content_disposition
from sleektiv.tests import tagged
from sleektiv.tests.common import HOST, new_test_user, get_db_name, BaseCase
from sleektiv.tools import config, file_path, parse_version
from sleektiv.addons.test_http.controllers import CT_JSON

from sleektiv.addons.test_http.utils import TEST_IP
...
class TestHttpMisc(TestHttpBase):
    ...
    def test_misc1_reverse_proxy(self):
        # client <-> reverse-proxy <-> sleektiv
        client_ip = '127.0.0.16'
        reverseproxy_ip = gethostbyname(HOST)
        host = 'mycompany.sleektiv.com'
    ...
    def test_misc2_local_redirect(self):
        def local_redirect(path):
            fake_req = sleektiv.tools.misc.DotDict(db=False)
            return sleektiv.http.Request.redirect(fake_req, path, local=True).headers['Location']
    ...
    def test_misc3_is_static_file(self):
        ...
        self.assertEqual(root.get_static_file(f'sleektiv.com/{uri}', host='sleektiv.com'), path, "Valid file with valid host")
        self.assertEqual(root.get_static_file(f'http://sleektiv.com/{uri}', host='sleektiv.com'), path, "Valid file with valid host")
        ...
        self.assertIsNone(root.get_static_file(f'sleektiv.com/{uri}'), "No host allowed")
        self.assertIsNone(root.get_static_file(f'http://sleektiv.com/{uri}'), "No host allowed")
    ...
    def test_misc5_geoip(self):
        ...
        headers = {
            ...
            'X-Forwarded-Host': 'sleektiv.com',
            'X-Forwarded-Proto': 'https'
        }
        with patch.dict(sleektiv.tools.config.options, {'proxy_mode': True}):
...
class TestHttpEnsureDb(TestHttpBase):
    ...
    def test_ensure_db1_grant_db(self):
        ...
        self.assertEqual(sleektiv.http.root.session_store.get(res.cookies['session_id']).db, 'db0')
    ...
    def test_ensure_db3_change_db(self):
        ...
        new_session = sleektiv.http.root.session_store.get(res.cookies['session_id'])
    ...
    def test_ensure_db4_unicode(self):
        ...
        self.assertEqual(
            sleektiv.http.root.session_store.get(res.cookies['session_id']).db,
            'basededonnée1')
