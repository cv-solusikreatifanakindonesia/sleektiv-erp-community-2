...
re._MAXCACHE = 4096  # default is 512, a little too small for sleektiv
...
def report_configuration():
    ...
    """
    config = sleektiv.tools.config
    _logger.info("Sleektiv ERP Community Version 2 | software version %s", __version__)
...
def export_translation():
    ...
    with open(config["translate_out"], "wb") as buf:
        ...
        with registry.cursor() as cr:
            sleektiv.tools.translate.trans_export(config["language"],
...
def import_translation():
    ...
    registry = sleektiv.modules.registry.Registry.new(dbname)
    with registry.cursor() as cr:
        translation_importer = sleektiv.tools.translate.TranslationImporter(cr)
        translation_importer.load_file(config["translate_in"], config["language"])
        translation_importer.save(overwrite=overwrite)

def main(args):
    check_root_user()
    sleektiv.tools.config.parse_config(args, setup_logging=True)
    check_postgres_user()
...
class Server(Command):
    ...
    def run(self, args):
        sleektiv.tools.config.parser.prog = f'{Path(sys.argv[0]).name} {self.name}'
        main(args)
