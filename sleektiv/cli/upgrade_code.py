...
* addon: the sleektiv addon in which the file is;
...
try:
    ...
    from sleektiv.modules import initialize_sys_path
    from sleektiv.tools import config, parse_version
except ImportError:
    # Assume the script is directy executed (by opposition to be
    # executed via sleektiv-bin), happily release/parse_version are
    # standalone so we can hack our way there without importing sleektiv
...
class UpgradeCode(Command):
    """ Rewrite the entire source code using the scripts found at /sleektiv/upgrade_code """
    name = 'upgrade_code'
    ...
    def __init__(self):
        self.parser = argparse.ArgumentParser(
            ...
            description=__doc__.replace('/sleektiv/upgrade_code', str(UPGRADE)),
    ...
    def run(self, cmdargs):
        ...
        if initialize_sys_path:
            ...
            initialize_sys_path()
            options.addons_path = sleektiv.addons.__path__
        else:
